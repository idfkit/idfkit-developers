...
Both currently declare `conformance-2026.6`. Neither exposes that value at
runtime yet, so a caller who wants to know what a build claims reads the config
file the way CI does.

The two libraries release on independent schedules, and matching release numbers
are never evidence of agreement. The declared level is, and it is the stronger
claim precisely because something outside both libraries tested it.
