...
# --8<-- [start:quickstart]
from idfkit import load_idf, write_idf, save_idf, save_epjson
...
save_epjson(doc, "out.epJSON")

# Or get the text back instead
idf_text = write_idf(doc)
# --8<-- [end:quickstart]


# --8<-- [start:output-type]
save_idf(doc, "out.idf", output_type="compressed")
# --8<-- [end:output-type]


# --8<-- [start:preserve]
from idfkit import load_idf, write_idf, save_idf

doc = load_idf("building.idf", preserve_formatting=True)
doc["Zone"]["Office"].x_origin = 10.0
save_idf(doc, "building_modified.idf")
...
save_epjson(doc, "building.epJSON")  # IDF → epJSON
# --8<-- [end:convert]


# --8<-- [start:convert-back]
from idfkit import load_epjson, save_idf

doc = load_epjson("building.epJSON")
save_idf(doc, "building.idf")  # epJSON → IDF
# --8<-- [end:convert-back]
...
for wwr in (0.2, 0.3, 0.4, 0.5):
    ...
    set_wwr(doc, wwr)
    save_idf(doc, out_dir / f"wwr_{int(wwr * 100)}.idf", output_type="compressed")
# --8<-- [end:batch]
...
save_idf(doc, "out.idf")  # latin-1
# --8<-- [end:mistake-encoding-good]


# --8<-- [start:mistake-compressed-good]
save_idf(doc, "out.idf", output_type="standard")  # lossless (default)
# or
save_idf(doc, "out.idf", output_type="compressed", preserve_formatting=False)
